...
Vmax = (1/1.602E-19) *1E10                           #'Infinite' potential
...
def plotter(x, y, n, Energy, y_name):
    ...
    plt.plot(x, y)
    plt.xlim(-2.5*half_width_box, 2.5*half_width_box)
...
def Schro_Infinite_Well(m):
    ...
    E_estimate = Energy_finder(Runge_Kutta, 0, -1, 1E-15, ((m**2)/2)*E_initial_guess, m)
...
def Schro_Finite_Well(m):

    E_estimate = Energy_finder(Runge_Kutta, 0, -1, 1E-15, ((m**2)/2)*E_initial_guess, m)
    x_val, y_val, h = Runge_Kutta(E_estimate, ODE_f_x, ODE_g_x, 1, 2.2*half_width_box, m, True, 100)
    #return E_estimate
